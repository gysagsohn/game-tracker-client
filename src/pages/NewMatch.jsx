...
export default function NewMatchPage() {
  ...
  // Players being added to this match
  // We pre-seed with "me" row (locked / not removable)
  const displayName =
    `${user?.firstName || ""} ${user?.lastName || ""}`.trim() || user?.email || "Player";
  ...
  const [players, setPlayers] = useState([
    {
      ...
      result: "Win",
      score: "",
      invited: false, // creator doesn’t need invite flag
      confirmed: true, // your backend auto-confirms creator
    },
  ]);
  ...
  useEffect(() => {
    ...
    async function loadFriends() {
      ...
      try {
        ...
        // Not fatal for creating a match; show a soft toast
  ...
  // If we arrived from /games/new and got a game in state, you can auto-select it
  useEffect(() => {
    const gid = location.state?.justAddedGameId;
    if (!gid) return;
    // GameSelect fetches games internally; once it lists, it should allow selecting
    // If your GameSelect accepts an id value, you can pass a shape: {_id: gid}
    // Otherwise, the user can just pick it—keeping this simple & robust.
  }, [location.state]);

  const friendOptions = useMemo(() => {
    const already = new Set(
      players
        .map((p) => (p.user ? String(idOf(p.user)) : null))
        .filter(Boolean)
    );
      ...
      rows.map((r, i) =>
        i === idx
          ? {
              ...r,
              [field]:
                field === "score" ? (val === "" ? "" : Number(val)) : field === "invited" ? !!val : val,
            }
          : r
      )
    );
  ...
  function addFriend() {
    ...
    setPlayers((rows) => [
      ...rows,
      {
        kind: "user",
        user: friend,
        name: `${friend.firstName || ""} ${friend.lastName || ""}`.trim() || friend.email || "Player",
        email: friend.email || "",
        result: "",
        score: "",
        invited: false,
  ...
  function addGuest() {
    ...
    setPlayers((rows) => [
      ...rows,
      {
        ...
        confirmed: true, // guests are auto-confirmed by backend
      },
    ]);
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // If API returns the new _id, take user to detail
      if (created?._id) {
      ...
      <Card className="p-6 max-w-3xl mx-auto">
        ...
        <form onSubmit={onSubmit} className="grid gap-5">
          ...
          <div>
            ...
            <div className="mt-1">
              <Link to="/games/new" className="underline text-sm" style={{ color: "var(--color-cta)" }}>
          ...
          <div>
                    ...
                    <tr key={idx} className="border-t" style={{ borderColor: "var(--color-border-muted)" }}>
                      ...
                      <td className="py-2 pr-2">
                        {p.kind === "self" ? (
                          ...
                          <button type="button" className="btn btn-sm" onClick={() => removePlayer(idx)}>
                            Remove
                          </button>
                        )}
                      </td>
            ...
            <div className="mt-3 grid gap-2 sm:flex sm:items-end">
              <div className="sm:w-80">
                ...
                >
                  <option value="">{friendsLoading ? "Loading friends…" : "Select a friend…"}</option>
                  {friendOptions.map((f) => (
              ...
              <Button type="button" className="btn-sm" onClick={addFriend} disabled={!friendIdToAdd}>
                Add friend
              </Button>
            </div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              ...
              <div>
                <label className="mb-1 block text-sm text-secondary">Guest email (optional)</label>
                <Input
                  value={guest.email}
                ...
                <label className="mt-1 block text-xs">
                  <input
                    ...
                    onChange={(e) => setGuest((g) => ({ ...g, invited: e.target.checked }))}
                  />
                  Send invite email
                </label>
              </div>
          ...
          <div className="flex gap-3">
            <Button type="submit" loading={saving} disabled={disableSave} className="w-full">
              Save Match
            </Button>
