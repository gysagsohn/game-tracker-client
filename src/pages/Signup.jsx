...
export default function SignupPage() {
  ...
  const [errors, setErrors] = useState({ firstName: "", lastName: "", email: "", password: "", confirm: "" });
  ...
  const onChange = (e) => {
    ...
    if (name === "email") setErrors((er) => ({ ...er, email: isEmail(value) ? "" : "Enter a valid email address." }));
    if (name === "firstName") setErrors((er) => ({ ...er, firstName: value.trim() ? "" : "First name is required." }));
    if (name === "lastName") setErrors((er) => ({ ...er, lastName: value.trim() ? "" : "Last name is required." }));
    if (name === "password") {
      const res = validatePasswordStrict(value);
      setErrors((er) => ({ ...er, password: res.ok ? "" : res.message, confirm: confirm === value ? "" : "Passwords do not match." }));
    }
  };
  ...
  const submitDisabled = loading || Object.values(errors).some(Boolean) || !form.firstName || !form.lastName || !form.email || !form.password || !confirm;
  ...
  return (
    <div className="min-h-screen bg-default flex items-center justify-center p-6">
      <div className="card shadow-card p-6 bg-card rounded-[var(--radius-standard)] w-full max-w-md text-center">
        ...
        <div aria-live="polite" className="sr-only">{formError || Object.values(errors).find(Boolean) || ""}</div>
        ...
        <form onSubmit={onSubmit} className="space-y-4 text-left">
          ...
          {errors.firstName && <p className="text-sm mt-1" style={{ color: "var(--color-warning)" }}>{errors.firstName}</p>}

          <Input label="Last name" name="lastName" value={form.lastName} onChange={onChange} required />
          {errors.lastName && <p className="text-sm mt-1" style={{ color: "var(--color-warning)" }}>{errors.lastName}</p>}
